...
function printAscii(node, showPlusMinus=false, forceMultiplySign=false, flattenUsed=true) {
  if (flattenUsed) {
...
// (The + - is needed to support the conversion of subtraction to addition of
// negative terms. See flattenOperands for more details if you're curious.)
function printLatex(node, showPlusMinus=false, forceMultiplySign=false, flattenUsed=true) {
  if (flattenUsed) {
    node = flatten(node.cloneDeep());
  } else {
    node = (node.cloneDeep());
