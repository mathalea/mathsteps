...
describe('basic addition collect like terms, no exponents or coefficients', function() {
  const tests = [
    ...
    ['y + nthRoot(x, 2) + 4y + nthRoot(x, 2)', '(nthRoot(x, 2) + nthRoot(x, 2)) + (y + 4 y)'],
...
describe('collect like terms with exponents and coefficients', function() {
  const tests = [
    ...
    ['y + 5 + z^2', 'y + 5 + z ^ 2'],
    ['2x^2 + x + x^2 + 3x', '(2 x ^ 2 + x ^ 2) + (x + 3 x)'],
    ['nthRoot(2)^3 + nthRoot(2)^3 - 6x', '(nthRoot(2) ^ 3 + nthRoot(2) ^ 3) - 6 x'],
    ['4x + 7 * nthRoot(11) - x - 2 * nthRoot(11)', '(7 * nthRoot(11) - 2 * nthRoot(11)) + (4 x - x)'],
  ];
  tests.forEach(t => testCollectLikeTerms(t[0], t[1], t[2]));
});
