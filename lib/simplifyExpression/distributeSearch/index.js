...
import { rearrangeCoefficient } from '../basicsSearch/rearrangeCoefficient.js';
...
function expandBase (node) {
  ...
  if (Node.Type.isFunction(base, 'nthRoot')) {
    ...
    const acarre = Node.Creator.operator('^',[base.args[0],exponent]);
    const bcarre = Node.Creator.operator('^',[base.args[1],exponent]);
    const doubleproduit = Node.Creator.operator('*',[ Node.Creator.constant(2),Node.Creator.operator('*',base.args)]);
    const expandedNode = Node.Creator.operator('+',[acarre,doubleproduit,bcarre]);
